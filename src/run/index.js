...
// Adjust the time interval for the first loop run between threads to avoid request spam (in seconds)
const DELAY_ACC = 10;
// Set the maximum number of reconnection attempts when proxy fails, if exceeded, the account will stop running and the error will be logged
const MAX_RETRY_PROXY = 20;
// Set the maximum number of login attempts when login fails, if exceeded, the account will stop running and the error will be logged
const MAX_RETRY_LOGIN = 20;
// Set the time NOT to play the game to avoid server error periods. For example, enter [1, 2, 3, 8, 20] to not play during hours 1, 2, 3, 8, 20
...
const run = async (user, index) => {
  ...
  while (true) {
    // Retrieve data from zuydd server
    if (database?.ref) {
    ...
    while (!isProxyConnected) {
      ...
      if (ip === -1) {
        user.log.logError(
          "Proxy error, check proxy connection, will try to reconnect after 30s"
        );
        countRetryProxy++;
    ...
    if (user.database?.skipHandleTask) {
      user.log.log(
        colors.yellow(
          `Temporarily skipping tasks due to server error (will automatically resume when server is stable)`
        )
      );
    } else {
...
if (IS_SHOW_COUNTDOWN && users.length) {
    ...
    if (isPauseAll) {
      ...
      process.stdout.write(
        colors.white(
          `[${dayjs().format(
            "DD-MM-YYYY HH:mm:ss"
          )}] All threads completed, waiting time: ${colors.blue(
  ...
  process.on("SIGINT", () => {
    console.log("");
    process.stdout.write("\x1b[K"); // Clear the current line from cursor to end of line
    process.exit(); // Exit the process
  });
}

setInterval(() => {}, 1000); // To keep the script running