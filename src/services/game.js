...
class GameService {
  ...
  async playGame(user, delay) {
    try {
      ...
      if (data) {
        user.log.log(
          `Starting the game, will end and receive rewards after: ${colors.blue(
            delay + "s"
          )}`
        );
        return data.gameId;
  ...
  async claimGame(user, gameId, eligibleDogs) {
    let points = generatorHelper.randomInt(169, 229);
    let dogs = 0;
    if (eligibleDogs) {
      points = generatorHelper.randomInt(159, 193);
      dogs = generatorHelper.randomInt(5, 10) * 0.1;
    }
    const payload = await this.createPayload(user, gameId, points, dogs);
  ...
  async createPayload(user, gameId, points, dogs) {
    const servers =
  ...
  getMinutesUntilNextStart(times) {
    ...
    // Calculate minutes from current time to next start time
  ...
  async handleGame(user, playPasses, timePlayGame) {
    ...
    if (isInTimeRange) {
      ...
      user.log.log(
        `${colors.blue(playPasses + " plays")} remaining ${colors.magenta(
          `[${textDropDogs}]`
        )}`
      );
      ...
      if (playPasses > 0)
        user.log.log(colors.magenta("All game plays have been used"));
      return -1;
    } else {
      const minutesUntilNextStart = this.getMinutesUntilNextStart(timePlayGame);
      user.log.log(
        colors.yellow(
          `Game play is not allowed during this time period, next play in: ${colors.blue(
