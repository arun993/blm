...
class AuthService {
  ...
  async handleLogin(user) {
    ...
    user.log.logError(
      "Login process failed, please check your account information (you may need to get a new query_id). The system will try to log in again after 60s"
    );
    return {
      status: 0,
      profile: null,
  ...
  async getProfile(user) {
    try {
      ...
      user.log.logError(
        `Failed to get account information: ${error.response?.data?.message}`
      );
      return null;
    }
  }
  ...
  async reconnect(user) {
    ...
    // user.log.logError(
    //   "Reconnection process failed, please check your account information (you may need to get a new query_id)"
    // );
    return 0;
  }
  ...
  async handleAfterReconnect(user, info) {
    ...
    // const profile = await this.getProfile(user);
    // if (profile) {
    //   user.log.logSuccess("Reconnection successful");
    // }
  }
}
