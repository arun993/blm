...
class FarmingClass {
  ...
  async startFarming(user) {
    try {
      ...
      if (data) {
        user.log.log(
          `Farming started, waiting to claim after: ${colors.blue("480 minutes")}`
        );
        return true;
      } else {
  ...
  async claimFarming(user, balance) {
    try {
      ...
      if (data) {
        ...
        throw new Error(`Farming claim failed: ${data.message}`);
      }
    } catch (error) {
      user.log.logError(
        `Farming claim failed: ${error.response?.data?.message}`
      );
      return false;
    }
  }
